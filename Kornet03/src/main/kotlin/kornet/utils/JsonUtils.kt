...
object JsonUtils {
	...
	class TransportJsonSerializer : JsonSerializer<Tank> {
	...
	fun serialize() {
		val gson: Gson =
			GsonBuilder().registerTypeHierarchyAdapter(Tank::class.java, TransportJsonSerializer()).setPrettyPrinting()
				.create()
	...
	fun deserialize() {
		val gson: Gson =
			GsonBuilder().registerTypeHierarchyAdapter(Tank::class.java, TransportJsonSerializer()).create()
