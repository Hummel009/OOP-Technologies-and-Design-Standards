...
object StandardUtils {
	fun convertXmlToJson() {
		...
		val gson = GsonBuilder().registerTypeHierarchyAdapter(CarBasic::class.java, JsonUtils.Serializer()).setPrettyPrinting().create()
	...
	fun loadDefaultList(): MutableList<Transport> {
		...
		transports.add(Decorator(CarLadaImproved(15500, "Yellow").setImprovement("Sunroof") as Transport))

		transports.add(Adapter(CarPeugeot()))
		transports.add(CarVolkswagenImproved(19000, "Grey").setImprovement("Chromed Sportline") as CarBasic)
		transports.add(CarLadaImproved(7000, "Violet").setImprovement("Sedan Baklazhan") as CarBasic)
